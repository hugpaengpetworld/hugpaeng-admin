...
declare
  ...
  wrapper_request_hash text;
  wrapper_key_id uuid;
  existing_wrapper_key public.idempotency_keys%rowtype;
...
begin
  ...
  wrapper_request_hash := encode(extensions.digest(
  ...
  where key.tenant_id = target_booking.tenant_id
    and key.scope = 'ROOM_PLAN_CHECK_IN'
  ...
  returning id into wrapper_key_id;

  if wrapper_key_id is null then
    select key.* into existing_wrapper_key
    ...
    if existing_wrapper_key.request_hash <> wrapper_request_hash then
      raise exception using errcode = '22023', message = 'IDEMPOTENCY_CONFLICT';
    end if;
    return existing_wrapper_key.result
  ...
  child_idempotency_key := encode(extensions.digest(
    convert_to(p_idempotency_key || ':check-in-booking', 'UTF8'),
    'sha256'
  ), 'hex');

  response_payload := public.check_in_booking(
    p_booking_id,
    p_room_id,
    p_deposit_satang,
    p_notes,
  ...
  update public.idempotency_keys key
  set result = response_payload
  where key.id = wrapper_key_id;
...
comment on function public.check_in_room_booking(
  ...
  'Atomically approves and checks in a held room booking with wrapper-level idempotency across the approval version transition.';
