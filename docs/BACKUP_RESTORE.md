...
The workflow creates a role dump plus `public` schema/data dumps on the encrypted ephemeral runner, records only SHA-256 evidence, and never uploads the dumps as Actions artifacts. The explicit `--schema public` boundary prevents Supabase-managed `storage` rows (including migration-created buckets) from being replayed as application rows. It then destructively rebuilds only the hard-locked restore target from reviewed migrations without seed data. Before replaying source application data, it transactionally truncates every target `public` table so migration-created reference rows cannot conflict with their restored equivalents; any replay error rolls back both the truncation and restore. It then reconciles the complete `public` schema and every public table fingerprint, verifies Auth create/login/delete, runs all database integration workflows, verifies fixture cleanup, and removes the temporary dumps even when a step fails.

For this CLEAN staging drill, Storage contains no uploaded objects. The reviewed migrations recreate the private `tenant-assets` bucket and its policies, so the SQL restore must not replay `storage.buckets`. A later drill with uploaded objects must additionally export, restore, and checksum the object manifest and binaries; the `public` PostgreSQL dump intentionally excludes Storage metadata, and PostgreSQL dump files do not contain Storage object binaries.
